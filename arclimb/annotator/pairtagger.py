...
class PointItem(BaseItem):
    ...
    def setCorrespondence(self, corr: 'CorrespondenceItem'):
...
class CorrespondenceItem(BaseItem):
    ...
    def __init__(self, sourceNode: PointItem, destNode: PointItem):
        ...
        self.dest = destNode
        self.source.setCorrespondence(self)
        self.dest.setCorrespondence(self)
        self.adjust()

    def getModel(self):
        return Correspondence(self.source.getModel(), self.source.getModel())
    ...
    def paint(self, painter, option, widget):
        ...
        assert self.source is not None and self.dest is not None
...
class ImagePairEditor(QtGui.QGraphicsView):
    ...
    def keyPressEvent(self, event):
        if self._is_inserting and event.key() == Qt.Key_Escape:
            self.stopInsertion()
...
class ImagePairEditorDialog(QtGui.QDialog):
    def __init__(self, image1, image2, parent = None):
        ...
        self.editor = ImagePairEditor(self, image1, image2)
...
if __name__ == '__main__':
    ...
    corr, accepted = ImagePairEditorDialog.run(images[0], images[1])
    print(corr)